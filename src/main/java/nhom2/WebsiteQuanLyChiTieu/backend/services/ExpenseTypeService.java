...
import nhom2.WebsiteQuanLyChiTieu.data.ExpenseTypeRepository;
...
public class ExpenseTypeService {
    ...
    public ExpenseType save(ExpenseType entity) throws ExpenseTypeAlreadyExistsException {
        if (expenseTypeRepository.existsByExpenseCategoryIgnoreCase(entity.getExpenseCategory())){
            throw new ExpenseTypeAlreadyExistsException("Loại chi phí '" + entity.getExpenseCategory() + "' đã tồn tại.");
        }
    ...
    public void init() {
        ...
        if (((Collection<?>) allExpenses).isEmpty()) {
            ExpenseType defaultExpenseType = new ExpenseType(null, "Trang chủ");
